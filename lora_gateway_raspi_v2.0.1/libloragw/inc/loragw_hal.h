...
/* lincentrator chipset-specific parameters */
/* to use array parameters, declare a local const and use 'if_chain' as index */
#define LGW_CHAN_NB_MAX		10		/* max number of IF+modem RX chains, in our solution, every IF chain is corresponding to a RF chain, to a SX127X */
...
#define LGW_FRQ_H	525000000
#define LGW_FRQ_L	137000000
...
enum lgw_radio_type_e {
	...
	LGW_RADIO_TYPE_SX1277,
	LGW_RADIO_TYPE_SX1278,
	LGW_RADIO_TYPE_SX1279
};

enum lgw_mode_type_e {
	LGW_MODE_TYPE_NONE,
	LGW_MODE_TYPE_LORA,
	LGW_MODE_TYPE_FSK
};
...
struct lgw_conf_board_s {
	...
	uint8_t pins_DIO[6]; // 6 DIO pins, if not used, set as 0
	uint8_t pin_RST; // 1 RST pin, if not used, set as 0
	uint8_t pin_CS[10]; // pins for CS, (or for 4-16 decoder mm74hc154wm), if not used, set as 0
...
struct lgw_conf_rxrf_s {
	...
	enum lgw_mode_type_e	mode;			/*!> LORA or FSK */
...
struct lgw_pkt_rx_s {
	...
	uint32_t	count_us;	/*!> internal lincentrator counter for timestamping, 1 microsecond resolution */
...
void radio_irq_hanlder();
/**
@brief Connect to the LoRa lincentrator, reset it and configure it according to previously set parameters
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_start(void);

/**
@brief Stop the LoRa lincentrator and disconnect it
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_stop(void);
...
/**
@brief A non-blocking function that will fetch up to 'max_pkt' packets from the LoRa lincentrator FIFO and data buffer
...
/**
@brief Schedule a packet to be send
