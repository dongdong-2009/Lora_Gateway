...
Based on Semtech-Cycleo and lora_packet_forwarder, modified by linjinzhi, jz.lin@siat.ac.cn
*/
...
#define XERR_INIT_AVG       10// TODO: 128         /* nb of measurements the XTAL correction is averaged on as initial value */
...
static pthread_mutex_t mx_meas_up = PTHREAD_MUTEX_INITIALIZER; /* control access to the upstream measurements  控制对上游测量的访问。*/
...
static int parse_channel_configuration(const char * conf_file);
...
int parse_channel_configuration(const char * conf_file) {
	int i;
	const char conf_obj[] = "chan_conf";
	...
	if (conf == NULL) {
		...
		MSG("INFO: %s does contain a JSON object named %s, parsing channels parameters\n", conf_file, conf_obj);
	}

	// set configuration for RF chains
	for (i = 0; i < LGW_CHAN_NB_MAX; ++i) {
		memset(&rfconf, 0, sizeof(rfconf)); // initialize configuration structure
		sprintf(param_name, "chan_%i", i); // compose parameter path inside JSON structure
		...
		sprintf(param_name, "chan_%i.enable", i);
		...
		if (rfconf.enable == false) { // radio disabled, nothing else to parse
			...
			snprintf(param_name, sizeof param_name, "chan_%i.type", i);
			str = json_object_dotget_string(conf, param_name);
			if (!strncmp(str, "SX1255", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1255;
			} else if (!strncmp(str, "SX1257", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1257;
			} else if (!strncmp(str, "SX1272", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1272;
			} else if (!strncmp(str, "SX1276", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1276;
			} else if (!strncmp(str, "SX1277", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1277;
			} else if (!strncmp(str, "SX1278", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1278;
			} else if (!strncmp(str, "SX1279", 6)) {
				rfconf.type = LGW_RADIO_TYPE_SX1279;
			} else {
				MSG("WARNING: invalid radio type: %s (should be SX1255 or SX1257)\n", str);
			}
			snprintf(param_name, sizeof param_name, "chan_%i.tx_enable", i);
			val = json_object_dotget_value(conf, param_name);
			if (json_value_get_type(val) == JSONBoolean) {
			...
			bw = (uint32_t)json_object_dotget_number(conf, param_name);
			if      (bw <= 7800)   rfconf.bandwidth = BW_7K8HZ;
			else if (bw <= 15600)  rfconf.bandwidth = BW_15K6HZ;
			else if (bw <= 31200)  rfconf.bandwidth = BW_31K2HZ;
			else if (bw <= 62500)  rfconf.bandwidth = BW_62K5HZ;
			else if (bw <= 125000) rfconf.bandwidth = BW_125KHZ;
			else if (bw <= 250000) rfconf.bandwidth = BW_250KHZ;
			else if (bw <= 500000) rfconf.bandwidth = BW_500KHZ;
			else rfconf.bandwidth = BW_UNDEFINED;

			snprintf(param_name, sizeof param_name, "chan_%i.mode", i);
			str = json_object_dotget_string(conf, param_name);
			if (!strncmp(str, "LORA", 4)) {
				rfconf.mode = LGW_MODE_TYPE_LORA;
				snprintf(param_name, sizeof param_name, "chan_%i.spread_factor", i);
				sf = (uint32_t)json_object_dotget_number(conf, param_name);
				switch(sf) {
					case  7: rfconf.sf = DR_LORA_SF7;  break;
					case  8: rfconf.sf = DR_LORA_SF8;  break;
					case  9: rfconf.sf = DR_LORA_SF9;  break;
					case 10: rfconf.sf = DR_LORA_SF10; break;
					case 11: rfconf.sf = DR_LORA_SF11; break;
					case 12: rfconf.sf = DR_LORA_SF12; break;
					default: rfconf.sf = DR_UNDEFINED;
				}
				MSG("INFO: channel %i enabled (LORA, type %s), center frequency %u, RSSI offset %f, tx enabled %d, %u Hz bandwidth, SF %u\n", i, str, rfconf.freq_hz, rfconf.rssi_offset, rfconf.tx_enable, rfconf.bandwidth, sf);
			} else if (!strncmp(str, "FSK", 3)) {
				rfconf.mode = LGW_MODE_TYPE_FSK;
				snprintf(param_name, sizeof param_name, "chan_%i.datarate", i);
				rfconf.datarate = (uint32_t)json_object_dotget_number(conf, param_name);
				MSG("INFO: channel %i enabled (FSK, type %s), center frequency %u, RSSI offset %f, tx enabled %d, %u Hz bandwidth, datarate %u\n", i, str, rfconf.freq_hz, rfconf.rssi_offset, rfconf.tx_enable, rfconf.bandwidth, rfconf.datarate);
			} else {
				rfconf.mode = LGW_MODE_TYPE_NONE;
				MSG("WARNING: channel %i enabled, but invalid mode type: %s (should be LORA or FSK)\n", i,str);
			}
		}
...
int parse_gateway_configuration(const char * conf_file) {
	...
	// set board configuration
		...
		//set up pins configuration
		val =  json_object_get_value(conf,"pin_CS_0");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pin_CS[0] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_CS_1");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pin_CS[1] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_CS_2");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pin_CS[2] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_CS_3");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pin_CS[3] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_DIO_0");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pins_DIO[0] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_DIO_1");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pins_DIO[1] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_DIO_2");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pins_DIO[2] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_DIO_3");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pins_DIO[3] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_DIO_4");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pins_DIO[4] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_DIO_5");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pins_DIO[5] = (uint8_t)json_value_get_number(val);
		}
		val =  json_object_get_value(conf,"pin_RST");
		if(json_value_get_type(val) == JSONNumber){
			boardconf.pin_RST = (uint8_t)json_value_get_number(val);
		}
		// all parameters parsed, submitting configuration to the HAL
	...
	str = json_object_get_string(conf, "server_address");
	if (str != NULL) {
	...
	val = json_object_get_value(conf, "keepalive_interval");
	if (val != NULL) {
	...
	val = json_object_get_value(conf, "stat_interval");
	if (val != NULL) {
	...
	val = json_object_get_value(conf, "push_timeout_ms");
	if (val != NULL) {
	...
	val = json_object_get_value(conf, "forward_crc_valid");
	if (json_value_get_type(val) == JSONBoolean) {
		fwd_valid_pkt = (bool)json_value_get_boolean(val);
	}
	MSG("INFO: packets received with a valid CRC will%s be forwarded\n", (fwd_valid_pkt ? "" : " NOT"));
	val = json_object_get_value(conf, "forward_crc_error");
	if (json_value_get_type(val) == JSONBoolean) {
		fwd_error_pkt = (bool)json_value_get_boolean(val);
	}
	MSG("INFO: packets received with a CRC error will%s be forwarded\n", (fwd_error_pkt ? "" : " NOT"));
	val = json_object_get_value(conf, "forward_crc_disabled");
	if (json_value_get_type(val) == JSONBoolean) {
	...
	val = json_object_get_value(conf, "fake_gps");
	if (json_value_get_type(val) == JSONBoolean) {
	...
	val = json_object_get_value(conf, "beacon_period");
	if (val != NULL) {
	...
	val = json_object_get_value(conf, "beacon_freq_hz");
	if (val != NULL) {
	...
	val = json_object_get_value(conf, "autoquit_threshold");
	if (val != NULL) {
...
int main(void)
{
	...
	char *local_cfg_path = "/home/pi/remote-debugging/local_conf.json"; /* contain node specific configuration, overwrite global parameters for parameters that are defined in both */
	...
	if (access(debug_cfg_path, R_OK) == 0) { /* if there is a debug conf, parse only the debug conf */
		...
		parse_channel_configuration(debug_cfg_path);
		...
		parse_channel_configuration(global_cfg_path);
		...
		if (access(local_cfg_path, R_OK) == 0) {
			...
			parse_channel_configuration(local_cfg_path);
    ...
    if(system("sudo ntpd -gq") < 0) {
    	MSG("ERROR: update NTP time failed!\n");
    	exit(EXIT_FAILURE);
    }
    /* get timezone info */
    tzset();
    ...
    //if (gps_enabled == true) {
    if (gps_fake_enable == true) { // modified by linjinzhi
    	gps_enabled = true;
...
void thread_up(void) {
	...
	/* local timestamp variables until we get accurate GPS time 本地时间戳变量直到我们得到准确的GPS时间。 */
	...
	*(uint32_t *)(buff_up + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		radio_irq_handler(); // TODO: Hardware bug! DIOx can not generate irq correctly, so we manually invoke radio_irq_handler().
		...
		for (i=0; i < nb_pkt; ++i) {
			...
			/* basic packet filtering  mx_meas_up 控制对上游测量的访问 */
			pthread_mutex_lock(&mx_meas_up);
			meas_nb_rx_rcv += 1;
			switch(p->status) {
				case STAT_CRC_OK:
            ...
            if (ref_ok == true) {
				...
				if(clock_gettime(CLOCK_REALTIME, &fetch_time) == 0) {
					...
					snprintf(fetch_timestamp, sizeof(fetch_timestamp), "%04i-%02i-%02iT%02i:%02i:%02i.%06liZ", 1900, 1, 1, 0, 0, 0, 0);
				}
...
void thread_down(void) { POLLING SERVER AND ENQUEUING PACKETS IN JIT QUEUE
	int i; /* loop variables */
	
	/* configuration and metadata for an outbound packet  出站数据包的配置和元数据。*/
	...
	*(uint32_t *)(buff_req + 8) = net_mac_l;
    ...
    // NetID, the last two bytes from gateway_ID
    beacon_pkt.payload[0] = (lgwm & 0xFF);
    beacon_pkt.payload[1] = ((lgwm >> 8) & 0xFF);
    /* 2-5 : time (variable) */
    /* 6-7 : crc1 (variable) */
		...
		while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
            ...
            while (beacon_loop && (beacon_period != 0)) {
                ...
                if ((gps_ref_valid == true) && (xtal_correct_ok == true)) {
                    ...
                    if(gps_fake_enable) {
                    	beacon_pkt.count_us = next_beacon_gps_time.tv_sec*1000000 + next_beacon_gps_time.tv_nsec/1000;
                    } else {
                    ...
                    // beacon_pkt.freq_hz = (uint32_t)(xtal_correct * (double)beacon_freq_hz); // TODO: by linjinzhi
                    beacon_pkt.freq_hz = (uint32_t)((double)beacon_freq_hz);
			...
			if (i == 1) {
				...
				if (val != NULL) {
					...
					// added by linjinzhi
					if(gps_fake_enable) {
						txpkt.count_us = utc_tx.tv_sec * 1000000 + utc_tx.tv_nsec/1000;
					} else {
            ...
            if ((txpkt.freq_hz < LGW_FRQ_L) || (txpkt.freq_hz > LGW_FRQ_H)) {
                jit_result = JIT_ERROR_TX_FREQ;
                MSG("ERROR: Packet REJECTED, unsupported frequency - %u (min:%u,max:%u)\n", txpkt.freq_hz, LGW_FRQ_L, LGW_FRQ_H);
            }
        ...
        if (jit_result == JIT_ERROR_OK) {
            if (pkt_index > -1) {
                ...
                if (jit_result == JIT_ERROR_OK) {
                    ...
                    result = lgw_status(pkt.rf_chain, &tx_status);
...
void thread_gps(void) {
    ...
    while (!exit_sig && !quit_sig) {
    	...
    	if(gps_fake_enable) { // modified by linjinzhi, using fake GPS
    		wait_ms(5000); // update every 5 seconds
    	    struct timeval now;
    	    gettimeofday(&now,NULL);
    	    //clock_gettime(CLOCK_MONOTONIC,&now);
    	    struct tm *tt;
    	    tt = gmtime(&(now.tv_sec));
    	    double lat = reference_coord.lat;
    	    double longi = reference_coord.lon;
    	    double alt = reference_coord.alt;
    	    char checksum[2];

    	    if(cnt++%2) {
				// $xxRMC,time,status,lat,NS,long,EW,spd,cog,date,mv,mvEW,posMode*cs
				// $GPRMC,213920.000,A,4221.1129,N,07102.9146,W,0.00,,010207,,,A*6D
				nb_char = sprintf(serial_buff,"$GPRMC,%02i%02i%02i.%03i,A,%.04f,N,%.04f,E,0.00,,%02i%02i%02i,,,A*00",tt->tm_hour,tt->tm_min,tt->tm_sec,(int)(now.tv_usec/1000),\
						lat,longi,tt->tm_mday,(tt->tm_mon)+1,(tt->tm_year)+1900-2000);
    	    } else {
				...
				nb_char = sprintf(serial_buff,"$GPGGA,%02i%02i%02i.%03i,%.04f,N,%.04f,E,1,08,1.01,%.01f,M,48.0,M,,0000*00",tt->tm_hour,tt->tm_min,tt->tm_sec,(int)(now.tv_usec/1000),\
									lat,longi,alt);
    	    }

    		if(nmea_checksum(serial_buff,nb_char,checksum) < 0) {
    			MSG("WARNING: [gps] calculate checksum failed.\n");
    			continue;
    		}
    		sprintf(serial_buff+nb_char-2,"%c%c",checksum[0],checksum[1]);

    		// TODO: todel
    		//printf("nb_char = %d, NEMA String is %s\n",nb_char,serial_buff);
