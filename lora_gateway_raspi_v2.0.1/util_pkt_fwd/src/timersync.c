...
int get_concentrator_time(struct timeval *concent_time, struct timeval unix_time) {
    ...
    pthread_mutex_lock(&mx_timersync);
    ...
    MSG_DEBUG(DEBUG_TIMERSYNC, "           concentrator current time is %ld,%ld\n", local_timeval.tv_sec, local_timeval.tv_usec);
...
void thread_timersync(void) {
    ...
    while (!exit_sig && !quit_sig) {
        ...
        lgw_get_trigcnt(&timecount);
        concentrator_timeval.tv_sec = timecount / 1000000UL;
        concentrator_timeval.tv_usec = timecount - (concentrator_timeval.tv_sec * 1000000UL);
        ...
        struct timeval temp;
        get_concentrator_time(&temp,unix_timeval);
